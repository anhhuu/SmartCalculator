...
{
	public partial class frmBPTB32 : Form
	{
		public frmBPTB32()
		{
			InitializeComponent();
		...
		private void frmBPTB32_Load(object sender, EventArgs e)
		{
				...
				try
				{
					...
					if (dblA == 0)
					{
						if (dblB == 0)
						{
							...
							{
								double dblNghiem = -dblD / dblC;
								txtKQ.Text = "Bất phương trình có nghiệm:";
								txtX1.Text = "x ≤ " + dblNghiem.ToString();
							}
							else
							{
								double dblNghiem = -dblD / dblC;
						...
						{
							double dblDelta = dblC * dblC - 4 * dblB * dblD;
							if (dblDelta < 0)
							{
								txtKQ.Text = "Bất phương trình vô nghiệm";
							}
							...
							{
								double dblNghiem = -dblC / (2 * dblB);
								txtKQ.Text = "Bất phương trình có nghiệm:";
								txtX1.Text = "x = " + dblNghiem.ToString();
							}
							else
							{
								double dblNghiem1 = (-dblC - Math.Sqrt(dblDelta)) / (2 * dblB);
								double dblNghiem2 = (-dblC + Math.Sqrt(dblDelta)) / (2 * dblB);
								txtKQ.Text = "Nghiệm của bất phương trình là:";
								txtX1.Text = dblNghiem1.ToString() + " ≤ x ≤ " + dblNghiem2.ToString();
							}
						}
						else
						{
							...
							else
							{
								...
								double dblNghiem2 = (-dblC + Math.Sqrt(dblDelta)) / (2 * dblB);
								txtKQ.Text = "Nghiệm của bất phương trình là:";
								txtX1.Text = "x ≤ " + dblNghiem2.ToString() + " hoặc " + dblNghiem1.ToString() + " ≤ x";
							}
						}
					...
					else if (dblA > 0)
					{
						// Tìm biến số f
						// f = [(3c/a) - (b²/a²)] / 3
						double dblF = (((3 * dblC) / dblA) - ((dblB * dblB) / (dblA * dblA))) / 3;

						// Tìm biến số g
						// g = [(2b³/a³) - (9bc/a²) + (27d/a)] / 27
						double dblG = (((2 * dblB * dblB * dblB) / (dblA * dblA * dblA)) - ((9 * dblB * dblC) / (dblA * dblA)) + ((27 * dblD) / dblA)) / 27;
						...
						else if (dblH <= 0)
						{
							...
							// Tìm giá trị của P
							// P = (b/3a) * (-1)
							double dblP = (dblB / (3 * dblA)) * (-1);

							// Tính giá trị X1
							// X1 = 2j * cosine(k/3) - (b/3a)
							double dblCosinCuaKTren3 = Math.Cos(dblKTren3);
							double dblX1 = 2 * dblJ * dblCosinCuaKTren3 - (dblB / (3 * dblA));
